...
### 🏷️ 「当前充电状态」的功率格子改名为「平均功率」

- 那个大字一直显示的是所选时间范围内的**平均**功率，而旁边的交流输入电流、电压取的是**最新一刻**的值。于是用「电流 × 电压」一算，和功率对不上（例：32 A × 218 V ≈ 6.98 kW，功率却显示 6.68 kW）。
- 数字本身没算错，两个口径都有用，只是标题没说清。改名后各归各位：下方曲线的图例里同时能看到平均值和最新值。

### 📌 上游进展：直流快充数据导致 TeslaMate 4.1.x 起不来，官方已给出修复

- v1.10.0 提到过一个**上游**问题：用过直流快充的数据库升级到 TeslaMate 4.1.x 时，重算历史电量那一步会因数值溢出失败，表现为容器反复重启。当时的建议是「先别升」。
- 官方已确认并给出修复，**下一个 TeslaMate 版本起，即使数据里有这种值也不会再失败**。现在就想升的话，官方也给了绕过办法。
- 自查方法、绕过步骤和当前状态都整理进了 [升级 TeslaMate 到 4.1.x 时报 smallint out of range](TROUBLESHOOTING.md#tm411-smallint)。
...
### 🐛 修复机器上有旧容器残留时，升级会误报「升级失败」

- 升级和安装脚本要先确认 TeslaMate 自己的数据库迁移做完了，才敢装单位换算函数（抢在它前面装会让 TeslaMate 起不来）。判断的办法是去看 TeslaMate 容器的状态。
- 问题在于挑容器的方式：**它会把已经停掉的容器也算进来，并且优先挑最近创建的那一个**。所以你机器上只要还留着一个停掉的 TeslaMate 容器——改过名的旧部署、试过一次没删干净的、换过一次栈的——脚本就会去看那个死容器的状态，得出「TeslaMate 起不来」的结论，于是**跳过单位换算函数并以红色「✗ 升级失败」收场，而你真正在跑的 TeslaMate 一切正常**。
- 现在的做法是：能从 Docker Compose 直接问到就以它为准；问不到时，机器上有多个像是 TeslaMate 的容器就**不再猜**，而是把它们列出来告诉你，并按「确认不了」处理（宁可少装一组可选的显示函数，也不会在你的 TeslaMate 已经出问题时误判成正常而把事情弄得更糟）。只有一个候选时照常工作，TeslaMate 真的反复重启时那条保护也依然有效。三个入口脚本（一键安装、官方迁移、升级）和诊断脚本都已修好。

### 🎨 修复部分图表的颜色、单位、坐标轴没有生效

- 一批面板的曲线配色、单位、坐标轴范围其实一直没生效：配置里写的是曲线的中文显示名，而 Grafana 按顺序处理配置，轮到它时那条曲线还叫原始英文名，于是整条配置作废。另有几处是「功率[kW]」和实际的「功率 [kW]」差一个空格、「电量(%)」和「电量（%）」半角全角括号不一致。
- 受影响的地方：**行程详情**的「行驶数据」（功率现在有独立的 kW 坐标轴，电量类曲线不再和功率挤在同一根轴上）、「温度」（空调开关不再画在温度轴上）；**驾驶记录追踪**的「驾驶」（功率、电池加热器恢复指定配色，海拔独立成轴并从 0 起）；**快充曲线统计**的「最大功率」（横轴刻度带上 kW）；**行程统计（每次充电）**的能耗单位；**当前驾驶状态**和**当前充电状态**的功率配色。
- 英制单位（英里 / ft / °F）的用户此前拿不到海拔相关的配色和坐标轴设置，本版一并补上。
...
- 一道会真起容器，把「只有一个」「一个在跑一个停了」「只剩停掉的」等几种情况逐个喂给三个入口脚本各跑一遍，断言判断一致；这段检测代码在三处的写法也纳入逐字比对，改一处漏两处会当场拦下。
- 一道盯着「装进镜像的仪表盘有没有全部被检查到」这件事本身——之前它对某些镜像打包写法认不出来，会静默放过。
