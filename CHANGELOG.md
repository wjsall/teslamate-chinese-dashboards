...
- 现在改成优先看正在运行的容器；一个都没运行时才回头看停掉的（这样 TeslaMate 真的反复重启时，那条保护依然有效）。三个入口脚本（一键安装、官方迁移、升级）都已修好。
...
- 一道会真起两个容器（一个运行中、一个后创建且已停止），断言选中的是运行中那个——就是上面那个「误报升级失败」的形状。三个入口脚本里的这段检测代码也纳入了逐字比对，改一处漏两处会当场拦下。
