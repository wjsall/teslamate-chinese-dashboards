...
### 🚨 修复 v1.9.6 对老用户无效 —— 升级 TeslaMate 4.1.1 仍会起不来（本版最重要的一条）

- v1.9.6 修的是「本项目的两个数据库对象钉住了 TeslaMate 的充电费用字段，导致 TeslaMate 4.1.1 启动失败、容器反复重启」。但那一版只做了「今后不再创建其中那个对账视图」，**没有删掉旧版本已经建好的那一个**。
- 结果是：**全新安装的用户没问题，而从 v1.9.5 或更早升上来的用户装了 v1.9.6 之后，TeslaMate 4.1.1 照样起不来**——也就是绝大多数受影响的人。这是我们的疏漏，v1.9.6 没有解决你的问题，请升到本版。
- 本版安装时会删掉那个遗留视图。**如果你基于它建过自己的查询或视图，你的东西不会被删**——安装会跳过它，并明确告诉你这个视图正挡着 TeslaMate 升级、以及怎么处理。同名位置被换成表、物化视图，或视图归属其他数据库账号时，同样如此。
- 升级/安装脚本结尾的提示也修了：这件事以前只在中间打印一行，结尾仍是绿色的「✓ 升级完成」，而一个无关紧要的回算失败反倒会亮黄灯。现在它会让结尾横幅变色，并写清后果——不处理的话 TeslaMate 升到 4.1.1 会启动失败、行车和充电全部停止记录。处理完重跑一次，提示会自己消失。

### 🧪 三道新的自动检查

- 一道会真跑升级脚本，断言「有未处理的遗留对象时，结尾不能是绿色的成功横幅」。
- 一道把两个一键脚本的结尾提示接上真实数据库执行，逐条核对措辞和后果说明——此前只有静态检查，把判断条件改成恒真就能骗过去，而用户会重新看到绿色的「安装完成」。
- 兼容性检查新增「先装旧版本、再装当前版本」这条路径。之前它只验全新安装，这正是 v1.9.6 那个疏漏能溜过去的原因。

### ⬆️ 内置 Grafana 升级到 13.1.3（原 13.0.1）

这次升级主要是为了修掉一类**你自己的数据就能触发、而且不一定会报错**的问题：Grafana 13.0.1 在把面板 SQL 发给数据库之前会先剥掉注释，但它不认引号，会把**引号内文字**里的 `--` 也当成注释符。后果分三种：

- 收藏点名字里带 `--`（例如「地库 B2--3 号桩」）→ 「省钱分析」的 4 个面板直接报错，画不出来；
- 分时电价配置表单填错时，弹出的报错文字驴唇不对马嘴，看不出到底哪填错了；
- 最麻烦的一种：**面板正常显示、却是错数字**——不报错、不空白，只是数值不对。

13.1.3 换成了认引号的处理方式，这三类都不再发生，同时带上 8 个已公开安全漏洞的修复。升级前做了三轮完整回归（每轮 400+ 条面板查询），45+3 个仪表盘全部正常，分时电价配置表单所依赖的插件在新版上照常工作。

**有一处外观变化：面板标题栏变高了 8 像素**（Grafana 官方改动，无法关闭）。带标题的矮面板——3 格高及以下、全项目共 86 个，主要是首页那种「单个大数字」的小方块——图形区会相应变矮一点点，数据和功能不受影响。

升级方式和平常一样，见上面的[升级到最新版](README.md#upgrade-latest)；仪表盘和数据不需要任何额外操作。

## [v1.9.6] - 2026-08-16

### 🚨 修复升级 TeslaMate 4.1.1 后服务起不来

- TeslaMate 4.1.1 调整了充电费用字段的精度。本项目有两个数据库对象「钉」在这个字段上，导致 TeslaMate 启动时改不动它，**数据库升级中断、容器反复重启、行车和充电全部停止记录**。凡是装过分时电价功能的用户升级到 4.1.1 都会遇到，与你的数据无关。
- 本版起这两个对象不再钉住任何字段，升级可以正常完成。**已经卡住的用户装上本版即可**：升级脚本会先修好数据库对象，再让 TeslaMate 完成升级。
- 同时新增一道自动检查：每次发布前会尝试改动 TeslaMate 三张主表的每一个字段，只要本项目挡住了任何一个就不予发布。这类问题以后不会再等到用户升级时才发现。
- ⚠️ **另有一处需要你注意的上游问题**：TeslaMate 4.1.1 重算历史电量的那一步，在**用过直流快充**的数据库上会因数值溢出而失败（400V 下电流超过 81A 即触发，超充轻易超过）。这不是本项目能修的，已上报官方（[teslamate#5620](https://github.com/teslamate-org/teslamate/issues/5620)）。升级前可以先自查：
  ```sql
  SELECT count(*) FROM charges
  WHERE charger_actual_current::int * charger_voltage::int > 32767;
  ```
  结果不为 0，建议等官方修复后再升级 TeslaMate。

### 🐛 修复三相充电时「当前充电」页功率显示 0

- 「当前充电」页的功率曲线和功率数值，此前把相数写死成只认 1 相和 2 相，**三相充电（常见的 11kW 家用充电桩）一律算成 0 kW**。同一笔充电在「充电详情」页显示正常，两个页面对不上。
- 现在三相充电正确显示（例：16A / 230V / 三相 → 11.04 kW），单相和两相不受影响。

### 🐛 修复充电时「当前电池容量」「当前储能」显示充电前的数值

- 这两个数字在充电全程一直停在充电开始前的值，而不是实时值——不是偶尔不准，是整个充电过程都不对。
- 已按官方 4.1.1 的修法同步。

### 🐛 修复短时交流充电的平均功率显示「No data」

- 采样点很少的短时交流充电（插枪几分钟就拔），「充电详情」页的平均功率整格空白。官方在自己的数据库上实测约 10% 的充电会遇到。

### 📊 其它

- 「数据库信息」页的「未关闭充电」「未关闭行驶」两个计数现在可以点击，直接跳到对应的记录列表。
- 首页的「版本发布」面板已移除——它依赖的两个第三方转发服务都已停止提供服务，面板长期空白。官方 4.1.1 也删除了这个面板。
- 早期「默认电价」的历史迁移工具，在 TeslaMate 重算电量后仍能正确认出旧记录。

### 🩺 诊断结果更准确

- 诊断脚本会识别已关闭或使用外部服务的 MQTT、自定义数据库账号和库名，不再把这些合法配置判成部署失败。
- 端口检查改用容器的实际映射；已停止容器仍会被定位并显示日志，不会被本机其他同端口程序干扰。
- Tesla API 连通性从 TeslaMate 容器内部验证；插件同时核对版本和 Grafana 识别结果，分时电价会检查核心表与函数是否完整。
- 脚本继续保持只读，并在输出开头提醒公开分享前移除日志中的隐私信息。

## [v1.9.5] - 2026-08-01

### 🛠️ 排障支持校验兼容更多环境

- 排障提示的发布校验不再依赖额外安装的搜索工具，可在精简 Linux 环境中直接运行；`v1.9.5` 取代因该校验环境问题而未能发布镜像的 `v1.9.4`。
