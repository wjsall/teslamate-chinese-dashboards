...
- 分时电价的 SQL 里有一个视图直接引用了 TeslaMate 充电记录表的全部字段。等 TeslaMate 将来给那张表加字段（历史上加过好几次），重装分时电价就会在这个视图上报错中断，**后面一半内容（包括 11 个仪表盘都在用的费用函数）全部装不上，而且重跑还是同样的错**，不手动删视图就修不好。现在改成先删旧视图再建，不再受字段变化影响。
